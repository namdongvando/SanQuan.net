...
// tài khoản
//   public function SuapassTaiKhoan($row) {
//      $sql = "UPDATE `3ps`.`taikhoan` SET `pass` = MD5(MD5('{$row['pass']}')) WHERE `taikhoan`.`username` = '{$row['username']}' AND `macongty` = '{$row['macongty']}';";
//      self::$_Query = $sql;
//   }
...
//      
    ...
    function upload_image1($file, $folder) {
        if (!is_dir($folder)) {
            mkdir($folder, 0777);
        }
        $tienTo = "sanpham" . time() . rand(1, 2000);
        $ext = trim(substr($file["type"], 6, strlen($file["type"])));
        $name = basename($file['name'], '.' . $ext);
        $extension = "jpg|jpeg|png|gif|JPG|JPEG|PNG|GIF";
        if (strpos($extension, $ext) === false) {
    ...
    function upload_multi_image($file, $folder) {
        ...
        foreach ($file['name'] as $k => $name) {
            ...
            $listName[] = $file['name'][$k] = $folder . $tienTo . "-" . $k . '.' . $ext;
