...
class Model_TinhThanh extends Model_Adapter {
    ...
    function ThemTinhThanh($TinhThanh) {
        $sql = "INSERT INTO `" . table_fix . "tinhthanh` SET "
    ...
    function SuaTinhThanh($TinhThanh) {
        $sql = "UPDATE `" . table_fix . "tinhthanh` SET "
                . "`TenDiaChi` = '{$TinhThanh['TenDiaChi']}', "
                . "`MaDiaChiCha` = '{$TinhThanh['MaDiaChiCha']}', "
                . "`AnHien` = '1', "
    ...
    function XoaTinhThanh($TinhThanh) {
        $sql = "DELETE FROM `" . table_fix . "tinhthanh` WHERE `MaDiaChi` = '$TinhThanh'";
        $this->Query($sql);
        $this->Luu();
    }

    function DSTinh() {
        $sql = "select * from `" . table_fix . "tinhthanh` where `MaDiaChiCha` = 0 ";
        $this->Query($sql);
        $kq = $this->Tim();
        if ($this->GetNumRow() >= 1) {
            $kq = $this->fetchAll();
    ...
    function DSAllTinhAndHuyen() {
        $sql = "select * from `" . table_fix . "tinhthanh`";
        $this->Query($sql);
        $kq = $this->Tim();
        if ($this->GetNumRow() >= 1) {
            $kq = $this->fetchAll();
    ...
    function TimTinhHuyen($MaDiaChi) {
        $sql = "select * from `" . table_fix . "tinhthanh` where `MaDiaChi` = '$MaDiaChi'";
        $this->Query($sql);
        $kq = $this->Tim();
        if ($this->GetNumRow() >= 1) {
    ...
    function DSHuyenTheoTinh($TinhThanh) {
        $sql = "select * from `" . table_fix . "tinhthanh` where `MaDiaChiCha` = '{$TinhThanh}'";
        if ($TinhThanh == 0) {
            $sql = "select * from `" . table_fix . "tinhthanh` where `MaDiaChiCha` = '0'";
        }
        $this->Query($sql);
    ...
    function DSTinhNoiBat() {
        ...
        $sql = "select * from `" . table_fix . "tinhthanh` where `MaDiaChi` in ($lisTinh)";
        $this->Query($sql);
        $kq = $this->Tim();
        if ($this->GetNumRow() >= 1) {
            $kq = $this->fetchAll();
    ...
    function DSTinh0NoiBat() {
        $lisTinh = $this->ListTinhNoiBat();
        $sql = "select * from `" . table_fix . "tinhthanh` where `MaDiaChi` not in ($lisTinh) and `MaDiaChiCha` = 0";
        $this->Query($sql);
        $kq = $this->Tim();
        if ($this->GetNumRow() >= 1) {
            $kq = $this->fetchAll();
    ...
    function DSSanPham4TinhThanh($ThuocTinh, $ThuocTinh1) {
        $sql = "SELECT * FROM `" . table_fix . "sanpham` where `AnHien` = '1' AND (`GhiChu` LIKE '%{$ThuocTinh}%' OR `GhiChu` LIKE '%{$ThuocTinh1}%') order by `DoQuanTrong` DESC ";
        $this->Query($sql);
        return $this->fetchAll();
    }
    ...
    function DSSanPham4TinhThanh2DanhMuc($MaDanhMuc, $ThuocTinh, $ThuocTinh1) {
        ...
        $sql = "SELECT * FROM `" . table_fix . "sanpham` where {$DK} `AnHien` = '1'  order by `DoQuanTrong` DESC ";
        if ($ThuocTinh != "") {
            $sql = "SELECT * FROM `" . table_fix . "sanpham` where {$DK} `AnHien` = '1' AND (`GhiChu` LIKE '%{$ThuocTinh}%' OR `GhiChu` LIKE '%{$ThuocTinh1}%') order by `DoQuanTrong` DESC ";
        }
        $this->Query($sql);
        return $this->fetchAll();
    }
