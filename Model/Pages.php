...
class Model_Pages extends Model_Adapter {
    ...
    function __construct($Pages = NULL) {
        ...
        if ($Pages) {
            ...
            if ($this->LoaiPage == 1) {
                $this->LinkPages = BASE_DIR . $this->TieuDeKD . "/";
            } else {
                $this->LinkPages = BASE_DIR . $this->TieuDeKD . ".html";
            }
    ...
    function DSPages() {
        $sql = "SELECT * FROM `" . table_fix . "pages` ORDER BY `idPa` ASC";
        $this->Query($sql);
        return $this->fetchAll();
    }

    function DSPages4LoaiPage($Loai) {
        $sql = "SELECT * FROM `" . table_fix . "pages` where `LoaiPage` = '{$Loai}' ORDER BY `idPa` ASC";
        $this->Query($sql);
        return $this->fetchAll();
    }

    function DSPages4idGroup($idGroup) {
        $sql = "SELECT * FROM `" . table_fix . "pages` where `idGroup` = '{$idGroup}' ORDER BY `idPa` ASC";
        $this->Query($sql);
        return $this->fetchAll();
    }

    function TimPages($idPa) {
        $sql = "SELECT * FROM `" . table_fix . "pages` where `idPa` = '{$idPa}'";
        $this->Query($sql);
        return $this->fetchRow();
    }

    function TimPages4TieuDeKD($idPa) {
        $sql = "SELECT * FROM `" . table_fix . "pages` where `TieuDeKD` = '{$idPa}'";
        $this->Query($sql);
        return $this->fetchRow();
    }

    function GetIDPagesTieuDe($TieuDe) {
        $sql = "SELECT `idPa` FROM `" . table_fix . "pages` where `TieuDeKD` = '{$TieuDe}'";
        $this->Query($sql);
        $kq = $this->fetchRow();
        return $kq['idPa'];
    }
    ...
    function XoaPages($idPages, $Quyen = FALSE) {
        if ($Quyen) {
            $sql = "DELETE FROM `" . table_fix . "pages` WHERE `idPa` = {$idPages}";
            $this->Query($sql);
            return $this->Luu();
        }
        return $Quyen;
    ...
    function ThemPages($Pages) {
        ...
        $sql = "INSERT INTO `" . table_fix . "pages` "
    ...
    function SuaPages($Pages) {
        $Pages['TieuDeKD'] = $this->bodautv(strtolower(trim($Pages['TieuDe'])));
        $sql = "UPDATE `" . table_fix . "pages` SET "
                . "`TieuDe` = '{$Pages['TieuDe']}', "
