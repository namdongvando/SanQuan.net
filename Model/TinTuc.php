...
class Model_TinTuc extends Model_Pages {
    ...
    function __construct($TinTuc = NULL) {
        ...
        if ($TinTuc) {
            ...
            $this->title = htmlspecialchars_decode($TinTuc['title']);
            $this->keyword = htmlspecialchars_decode($TinTuc['keyword']);
            $this->description = htmlspecialchars_decode($TinTuc['description']);
            $this->MaDanhMuc = $TinTuc['MaDanhMuc'];
            $this->TenDanhMuc = $this->LayTenDanhMuc($this->MaDanhMuc);
            $this->TieuDe = htmlspecialchars_decode($TinTuc['TieuDe']);
            $this->TieuDeKhongDau = $TinTuc['TieuDeKhongDau'];
            $this->TomTat = $TinTuc['TomTat'];
            $this->NoiDungSua = htmlspecialchars_decode($this->NoiDungTinTuc($TinTuc['NoiDung']));
            $this->NoiDungHH = htmlspecialchars_decode($this->NoiDungTinTuc($TinTuc['NoiDung']));
            $this->NoiDung = htmlspecialchars_decode($TinTuc['NoiDung']);
    ...
    function ThemTinTuc($TinTuc) {
        $sql = "INSERT INTO `" . table_fix . "tin` SET "
                . "`IdTin` = '{$TinTuc['IdTin']}',"
                . "`title` = '{$TinTuc['title']}',"
    ...
    function DSTinTucTheoTen($TuKhoa) {
        $sql = "SELECT * FROM `" . table_fix . "tin` where `TieuDe` like '%{$TuKhoa}%' and `MaDanhMuc` > 0";
        $this->Query($sql);
        if ($this->GetNumRow() >= 1) {
    ...
    function DSTinTucAll($lang = 'vi') {
        $sql = "SELECT * FROM `" . table_fix . "tin` ORDER BY `Stt` ASC  ";
        $this->Query($sql);
        return $this->fetchAll();
    }
    function DSTinTuc($lang = 'vi') {
        $sql = "SELECT * FROM `" . table_fix . "tin` where  `MaDanhMuc` > 0  ORDER BY `Stt` ASC  ";
        $this->Query($sql);
    ...
    function TimTinTucCT($MaTin) {
        $sql = "SELECT * FROM `" . table_fix . "tin` where `idTin`= '{$MaTin}'";
        $this->Query($sql);
    ...
    function TimTieuDeTinTuc($TieuDe, $lang = 'vi') {
        $sql = "SELECT * FROM `" . table_fix . "tin` where `TieuDeKhongDau`= '{$TieuDe}' ";
        $this->Query($sql);
        return $this->fetchRow();
    }

    function TimTinTuc4TieuDe($TieuDe) {
        $sql = "SELECT * FROM `" . table_fix . "tin` where `TieuDe` like '%{$TieuDe}%' ";
        $this->Query($sql);
        return $this->fetchAll();
    }

    function DSAllTinTheoLoai($MaLoaiTin) {
        $sql = "SELECT * FROM `" . table_fix . "tin` where `MaDanhMuc`= '{$MaLoaiTin}' ORDER BY `Stt` ASC ";
        $this->Query($sql);
        return $this->fetchAll();
    }

    function DSTinTheoLoaiNoiBat($MaLoaiTin, $lang = 'vi') {
        $sql = "SELECT * FROM `" . table_fix . "tin` where `MaDanhMuc`= '{$MaLoaiTin}'  ORDER BY `Stt` ASC Limit 0,5 ";
        $this->Query($sql);
        return $this->fetchAll();
    }
    ...
    function TongDSTinTheoLoai($MaLoaiTin) {
        $sql = "SELECT count(*) as `TongTinTuc` FROM `" . table_fix . "tin` where `MaDanhMuc`= '{$MaLoaiTin}'";
        $this->Query($sql);
        $kq = $this->fetchRow();
    ...
    function DSTinTheoLoaiPT($MaLoaiTin, $lang = 'vi', $Vitri, $SoLuong) {
        $sql = "SELECT * FROM `" . table_fix . "tin` where `MaDanhMuc`= '{$MaLoaiTin}' and "
    ...
    function TongSoTinDangHienTheoLoai($LoaiSanPham, $lang = 'vi') {
        $sql = "SELECT count(*) as `TongSanPham` FROM `" . table_fix . "tin` where `lang` = '{$lang}' "
    ...
    function DSTinMoiNhat($SLTin, $lang = 'vi') {
        $sql = "SELECT * FROM `" . table_fix . "tin` where `AnHien` > 0  ORDER BY `Stt` ASC limit 0,{$SLTin}";
        $this->Query($sql);
        return $this->fetchAll();
    }

    function DSTinMoiNhat4Loai($SLTin, $Loai) {
        $sql = "SELECT * FROM `" . table_fix . "tin` where `MaDanhMuc` = '{$Loai}' ORDER BY `Stt` ASC limit 0,{$SLTin}";
        $this->Query($sql);
        return $this->fetchAll();
    }

    function DStTopTinMoiNhat($MaLoaiTin, $SLTin, $lang = 'vi') {
        $sql = "SELECT * FROM `" . table_fix . "tin` where `MaDanhMuc`= '{$MaLoaiTin}'  ORDER BY `Stt` ASC limit 0,{$SLTin}";
        $this->Query($sql);
        return $this->fetchAll();
    }

    function DS5TinNoiBat($SoLuong, $lang = 'vi') {
        $sql = "SELECT `idTin`,`urlHinh`,`TieuDe`,`TomTat`,`NgayDang`,`TieuDeKhongDau` FROM `" . table_fix . "tin` where `TinNoiBat` = '1'  ORDER BY `Stt` ASC limit 0,{$SoLuong}";
        $this->Query($sql);
        return $this->fetchAll();
    }

    function DS10TinNoiBat($lang = 'vi') {
        $sql = "SELECT * FROM `" . table_fix . "tin` where `lang` = '{$lang}' ORDER BY `Stt` ASC limit 0,10";
        $this->Query($sql);
        return $this->fetchAll();
    }

    function XoaTinTuc($idTin) {
        $sql = "DELETE FROM `" . table_fix . "tin` WHERE `idTin` = '{$idTin}'";
        $this->Query($sql);
        return $this->Luu();
    }

    function AnTinDangDuyet($idTin) {
        echo $sql = "UPDATE `" . table_fix . "tin_tam` SET "
        . "`AnHien` = '0' where "
        . "`idTin` = '{$idTin}'";
        $this->Query($sql);
        return $this->Luu();
    ...
    function LayTinTuDong($TinTuc) {
        $sql = "INSERT INTO `" . table_fix . "tin_tam` SET "
                . "`lang` = '{$TinTuc['lang']}',"
                . "`MaDanhMuc` = '{$TinTuc['MaDanhMuc']}',"
                . "`urlHinh` = '{$TinTuc['urlHinh']}',"
                . "`TieuDe` = '{$TinTuc['TieuDe']}',"
    ...
    function TimTinTuDong($idTin) {
        $sql = "SELECT * FROM `" . table_fix . "tin_tam` where `idTin` = '{$idTin}' and `AnHien` = '1'";
        $this->Query($sql);
        return $this->fetchRow();
    }

    function DSTinTuDong() {
        $sql = "SELECT * FROM `" . table_fix . "tin_tam` where `AnHien` = '1'";
        $this->Query($sql);
        return $this->fetchAll();
    }

    function DSTinHot($lang = 'vi') {
        $sql = "SELECT * FROM `" . table_fix . "tin` where `TinNoiBat` = '1'  ORDER BY `Stt` ASC ";
        $this->Query($sql);
        return $this->fetchAll();
    }

    function DSTimTinTuc($keyword, $lang = 'vi') {
        $sql = "SELECT `idTin`,`urlHinh`,`TieuDe`,`TomTat`,`AnHien`,`TinNoiBat`,`NgayDang`,`TieuDeKhongDau` FROM `" . table_fix . "tin` where `TieuDe` like '%$keyword%'  ORDER BY `Stt` ASC";
        $this->Query($sql);
        return $this->fetchAll();
    }

    function DSTimTinTucPT($keyword, $vitriHienTai, $soluong = 12, &$TongSanPham) {
        $sql = "SELECT count(*) as `TongSanPham` FROM `" . table_fix . "tin` where `TieuDe` like '%$keyword%'  ORDER BY `Stt` ASC";
        $this->Query($sql);
        $result = $this->Tim();
        if ($this->GetNumRow() >= 1) {
    ...
    function DSTinTucLoaiPT($vitri, $soluong = 8, $MaDanhMuc, &$TongSanPham) {
        $TongSanPham = 0;
        $sql = "SELECT count(*) as `TongSanPham` FROM `" . table_fix . "tin` where `MaDanhMuc` = '{$MaDanhMuc}' ORDER BY `Stt` ASC";
        $this->Query($sql);
        $result = $this->Tim();
        if ($this->GetNumRow() >= 1) {
        ...
        if ($TongSanPham == 0) {
            ...
            $sql = "SELECT * FROM `" . table_fix . "tin` where `MaDanhMuc` = '{$MaDanhMuc}' ORDER BY `Stt` ASC limit {$vitri},{$soluong} ";
            $this->Query($sql);
            if ($this->GetNumRow() >= 1) {
                $SanPham = $this->fetchAll();
                return $SanPham;
    ...
    function DSTinTucLoaiPT1($vitri, $soluong = 8, $MaDanhMuc, &$TongSanPham) {
        ...
        if ($TongSanPham <= 0) {
            ...
            $sql = "SELECT * "
                    . "FROM `" . table_fix . "tin` where `MaDanhMuc` = '{$MaDanhMuc}' ORDER BY `Stt` ASC limit {$vitri},{$soluong} ";
            $this->Query($sql);
            if ($this->GetNumRow() >= 1) {
                $SanPham = $this->fetchAll();
                return $SanPham;
