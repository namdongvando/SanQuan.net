...
class Model_DanhMuc extends Model_TinhThanh {
    ...
    function __construct($DanhMuc = NULL) {
        if ($DanhMuc) {
            ...
            $this->NoiDung = isset($DanhMuc['NoiDung'])?$DanhMuc['NoiDung']:'' ;
            $this->NoiDungHH = $_TinTuc->NoiDungTinTuc($this->NoiDung);
            $this->CapCha = isset($DanhMuc['CapCha'])?$DanhMuc['CapCha']:0 ;
            $this->UrlHinh = BASE_DIR . "public/img/images/" . $DanhMuc['UrlHinh'];
            $this->STT = isset($DanhMuc['STT'])?$DanhMuc['STT']:0;
            $this->LoaiDanhMuc = isset($DanhMuc['LoaiDanhMuc'])?$DanhMuc['LoaiDanhMuc']:0 ;
    ...
    public static function DanhMucs($CapCha = 0) {
        $sql = "SELECT * FROM `" . table_fix . "danhmuc` where `CapCha` = '{$CapCha}' ORDER BY `STT` ASC";
        $this->Query($sql);
        if ($this->GetNumRow() >= 1) {
    ...
    public function AllDanhMuc() {
        $sql = "SELECT `MaDanhMuc`,`TenDanhMuc`,`ThuocTinh`,`TenKhongDau`,`UrlHinh`,`CapCha` FROM `" . table_fix . "danhmuc` where 1 ORDER BY `STT` ASC";
        $this->Query($sql);
    ...
    public static function DanhMucTinTuc() {
        $sql = "SELECT * FROM `" . table_fix . "danhmuc` where `LoaiDanhMuc` = '{$this->LoaiDanhMuc}' ORDER BY `STT` ASC";
        $this->Query($sql);
        return $this->fetchAll();
    }
    ...
    function DSDanhMuc($maDanhMuc = 0) {
        $sql = "SELECT * FROM `" . table_fix . "danhmuc` where `CapCha` = '{$maDanhMuc}' ORDER BY `STT` ASC";
        $kq = $this->Query($sql);
        if ($this->GetNumRow() >= 1) {
            return $kq = $this->fetchAll();
        }
    ...
    function DSDanhMuc4ThuocTinh($key) {
        $sql = "SELECT * FROM `" . table_fix . "danhmuc` where `ThuocTinh` like '%{$key}%' ORDER BY `STT` ASC";
        $kq = $this->Query($sql);
        if ($this->GetNumRow() >= 1) {
            return $kq = $this->fetchAll();
        }
    ...
    function TimDanhCha($maDanhMuc) {
        ...
        $sql = "SELECT * FROM `" . table_fix . "danhmuc` where `MaDanhMuc` = '{$maDanhMuc}'";
        $this->Query($sql);
    ...
    function TimDanhMucTieuDe($TieuDeKD) {
        $sql = "SELECT * FROM `" . table_fix . "danhmuc` where `TenKhongDau` = '{$TieuDeKD}'";
        $this->Query($sql);
        return $this->fetchRow();
    }

    function DSSanPhamDanhMuc($MaDanhMuc) {
        $sql = "SELECT a.`MaSanPham`, a.`ChuSanPham`, a.`MaDanhMuc`, a.`TenSanPham`, a.`Gia`, a.`MoTa`, a.`UrlHinh`, a.`NgayDang`, a.`SoLanXem`,b.`MaDanhMuc`,b.`TenDanhMuc` FROM `" . table_fix . "sanpham` as a,`" . table_fix . "danhmuc` as b WHERE b.`MaDanhMuc` = '{$MaDanhMuc["MaDanhMuc"]}' and a.`MaDanhMuc`=b.`MaDanhMuc` and a.`ChuSanPham` ='{$MaDanhMuc["ChuSanPham"]}'  limit 0,6";
        $this->Query($sql);
        $kq = $this->Tim();
        if ($this->GetNumRow() >= 1) {
    ...
    function SuaDanhMuc($DanhMuc) {
        $sql = "UPDATE `" . table_fix . "danhmuc` SET "
    ...
    function SuaTTDanhMuc($MaDanhMuc, $Key, $value) {
        ...
        $sql = "UPDATE `" . table_fix . "danhmuc` SET `ThuocTinh` = '{$ThuocTinh}'  WHERE `MaDanhMuc` = '{$MaDanhMuc}'";
        $this->Query($sql);
        return $this->Luu();
    }

    function ThemDanhMuc($DanhMuc) {
        $sql = "INSERT INTO `" . table_fix . "danhmuc` SET "
                . "`TenDanhMuc` = '{$DanhMuc['TenDanhMuc']}',"
                . "`ThuocTinh` = '{$DanhMuc['ThuocTinh']}',"
                . "`LoaiDanhMuc` = '{$DanhMuc['LoaiDanhMuc']}',"
                . "`TenKhongDau` = '{$DanhMuc['TenKhongDau']}',"
    ...
    function XoaDanhMuc($DanhMuc, $Quyen = FALSE) {
        ...
        if ($Quyen) {
            ...
            if (!$DSDanhMucCon) {
                ...
                if ($kt == 1) {
                    $sql = "DELETE FROM `" . table_fix . "danhmuc` WHERE `MaDanhMuc` = '{$DanhMuc}'";
                    $this->Query($sql);
                    return $this->Luu();
                }
            }
    ...
    function DSDMRaoVat() {
        $sql = "SELECT * FROM `" . table_fix . "danhmuc`";
        $this->Query($sql);
        if ($this->GetNumRow() >= 1) {
            return $this->fetchAll();
        } else {
    ...
    function DSDanhMucTheoLoai($LoaiDM) {
        $sql = "SELECT * FROM `" . table_fix . "danhmuc` where `LoaiDanhMuc` = '{$LoaiDM}'";
        $this->Query($sql);
        return $this->fetchAll();
    }
